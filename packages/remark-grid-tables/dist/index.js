...
class TablePart {
  ...
  updateWithMainLine(line, isEndLine) {
    ...
    for (let c = 1; c < this.lastRow()._cells.length; c++) {
      const cell = this.lastRow()._cells[c]; // Only cells with rowspan equals can be merged
...
function extractTable(value, eat, tokenizer) {
  ...
  const possibleGridTable = markdownLines.map(line => trimEnd(line)); // Extract table
  ...
  for (; i < possibleGridTable.length; i++) {
    ...
    const realLine = markdownLines[i]; // line is in table

    if (totalMainLineRegex.exec(line)) {
      const isHeaderLine = headerLineRegex.exec(line);
      if (isHeaderLine && !hasHeader) hasHeader = true; // A table can't have 2 headers
...
function extractTableContent(lines, linesInfos, hasHeader) {
  ...
  for (let l = 0; l < lines.length; l++) {
    const line = lines[l]; // Get if the line separate the head of the table from the body

    const matchHeader = hasHeader & isHeaderLine(line) !== null; // Get if the line close some cells
  ...
  for (let p = 0; p < tableContent._parts.length; p++) {
    ...
    for (let r = 0; r < part._rows.length; r++) {
      ...
      for (let c = 0; c < row._cells.length; c++) {
        ...
        const tokenizedContent = tokenizer.tokenizeBlock(cell._lines.map(e => e.trim()).join('\n'), now);
        const cellElt = {
          type: 'tableCell',
...
function gridTableTokenizer(eat, value, silent) {
  ...
  const merged = merge(before, realGridTable, after); // Because we can't add multiples blocs in one eat, I use a temp block
...
function setSize(grid) {
  ...
  grid.forEach(row => {
    // Find the max
    const maxHeight = Math.max(...row.map(cell => cell.height)); // Set it to each cell
  ...
  grid[0].forEach((_, j) => {
    // Find the max
    const maxWidth = Math.max(...grid.map(row => row[j].width)); // Set it to each cell
