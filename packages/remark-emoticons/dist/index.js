...
module.exports = function inlinePlugin(ctx) {
  ...
  function locator(value, fromIndex) {
    let lowestMatch = -1; // Iterate on chars

    for (let c = 0; c < firstChars.length; c++) {
      const char = firstChars[c];
      let offset = 0;
      let match = -1; // Oftentimes, the first occurence is not good, so loop on all possible ones

      while (match === -1 && offset < value.length - 1) {
        match = value.indexOf(char, fromIndex + offset); // Also match uppercase
